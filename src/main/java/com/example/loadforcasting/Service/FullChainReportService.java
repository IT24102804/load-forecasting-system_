...
@Service
public class FullChainReportService {
    ...
    public byte[] exportFullChainReportPdf(Long costRunId) {
        if (costRunId == null || costRunId <= 0) {
            throw new IllegalArgumentException("costRunId is required.");
        }
        ...
        return buildPdf(costRunId, costRun, mixRun, loadRequest);
    }

    private byte[] buildPdf(Long costRunId, CostPredictionRun costRun, GenerationMixRun mixRun, LoadRequest loadRequest) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            Paragraph title = new Paragraph("ELECTRICITY FULL CHAIN REPORT", titleFont);
            ...
            addSectionHeader(metaTable, "REPORT METADATA", sectionFont);
            addRow(metaTable, "Cost Run Id", String.valueOf(costRunId), bodyFont);
            addRow(metaTable, "Exported", dtf.format(LocalDateTime.now()), bodyFont);
            ...
            addSectionHeader(forecastTable, "GENERATION MIX SETUP", sectionFont);
            ...
            addSectionHeader(costTable, "COST PREDICTION", sectionFont);
